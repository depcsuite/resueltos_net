...
using Libro = TiendaLibros.Models.Libro;
...
namespace TiendaLibros.Controllers
{
    ...
    public class LibrosController : Controller
    {
        ...
        public IActionResult Index()
        {
            ...
            var libroDto = LibrosMapper.MapearADto(libros);
        ...
        public IActionResult Index(string isbn)
        {
            ...
            var libroDto = LibrosMapper.MapearADto(libro);

            return View(libroDto);
        }

        private Libro GetLibroDB(string isbn)
        {
            return _context.Libros.Where(x => x.ISBN == isbn).FirstOrDefault();
        }

        [HttpPost]
        public IActionResult Index([FromBody] LibroDto libro)
        {

            //Simulo en libro recibido por post.
            libro = new LibroDto()
            {
                Ranking = 9,
                ISBN = "111-4565-22",
                ...
                Autor = "Robert Martin",
                Precio = 26500,
                PortadaLink = "111-4565-22.png",
                Detalles = new LibrosDetalleDto()
                {
                    FechaPublicacion = DateTime.ParseExact("10-02-2020", "MM-dd-yyyy", CultureInfo.CurrentCulture),
                    Genero = "Tecnologia",
                    Resumen = "Des aux vu jaunes et la roulant fond lyres énormes descendais brumes volets volets porteur des anses et balottant d'ineffables nacreux voiliers enlever douce les noirs tapages je du anglais"
                }
            };
        ...
        private void AgregarLibro(LibroDto libroDto)
        {

            var libro = LibrosMapper.Mapear(libroDto);
