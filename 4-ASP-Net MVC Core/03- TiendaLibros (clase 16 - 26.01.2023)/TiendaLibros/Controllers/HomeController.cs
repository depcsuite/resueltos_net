...
namespace TiendaLibros.Controllers
{
    public class HomeController : Controller
    {
        ...
        public HomeController(ILogger<HomeController> logger, LibrosDBContext context )
        {
            _logger = logger;
            _context = context;
        }
        ...
        //[HttpPost]
        //public IActionResult Index([FromBody] LibroDto libroDto)
        //{
        //    //Simula la entrada del post.

        //    var libro = GetLibro();

        //    var nuevoLibro = InsertarLibro(libro);
        ...
        private static Libro GetLibro()
        {
            var libro = new Libro()
            {
                Ranking = 9,
                ISBN = "111-4565-22",
                Titulo = "Codigo limpia",
                Autor = "Richard Martin",
                Precio = 23500,
                PortadaUri = "111-4565-22.png",
                Detalles = new LibroDetalle()
                {
                    ...
                    Resumen = "E come non come per lodato e al di che quegli intendo noi nel una come etterno carissime novellare pregato procuratore propria che intendo di speranza vita benignita segreto quale informati la sue piene cosa durare noia dallo mente che esse manifestamente cosa grazia da manifestamente alla il e le"
                }
            };
        ...
        //private static Libro InsertarLibro(Libro libro)
        //{
        ...
        public IActionResult Index()
        {
            ...
            //var libros = GetLibrosDB();

            //var librosDto = LibrosMapper.MapADto(libros);
        ...
        public IActionResult Detalles(string id)
        {
            ...
            var libroConDetalles = GetLibroByIsbn(id);
            return View(libroConDetalles);
        }

        private LibroDto GetLibroByIsbn(string isbn)
        {
            //Obtengo todos los libros.
            var libros = GetLibros();
            //var libro = GetLibroByIsbnDB(isbn);
            var libro = libros.Where(x => x.ISBN == isbn).FirstOrDefault();


            //Mapeo Entidad Libros con DTO Libros
            //var libroDto = LibrosMapper.MapADto(libro);

            return libro;
        }

        private Libro GetLibroByIsbnDB(string isbn)
        {
            return _context.Libros.Include(x => x.Detalles).FirstOrDefault();
        }

        /// <summary>
        /// Obtengo la lista de libros de la DB
        /// Version 1.
        /// </summary>
        /// <returns></returns>
        private List<Libro> GetLibrosDB()
        {
            var libros = _context.Libros.Include(x => x.Detalles).ToList();
        ...
        private static List<LibroDto> GetLibros()
        {
